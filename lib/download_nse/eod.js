...
function eod(options) {
  ...
  function download(opt, cb) {
    ...
    const time_now = moment().format('HH');
    const day_today = moment().format('ddd');
    let date_needed = moment();
    console.log('downloading eod > current hr:', time_now, ' current day:', day_today);
    ...
    console.log('downloading eod > url:', url);
